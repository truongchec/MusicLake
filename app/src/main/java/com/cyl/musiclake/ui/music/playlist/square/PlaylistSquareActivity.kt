...
class PlaylistSquareActivity : BaseActivity<BasePresenter<BaseContract.BaseView>>() {
    ...
    private var cateList = mutableListOf("Recommended", "Boutique", "Chinese", "Folk", "Rock", "Popular", "Old Style", "Japanese")
    ...
    private fun setupViewPager() {
        ...
        for (i in cateList.indices) {
            when (i) {
                0 -> mAdapter.addFragment(PlaylistFragment.newInstance("All"), cateList[i])
