        ...
        if (playlist.type == Constants.PLAYLIST_LOCAL_ID) {
            doAsync {
                ...
                uiThread {
                    ...
                    success.invoke("Playlist deleted successfully")
                }
            }
            return
        }
            ...
            listItems(items = items) { dialog, index, text ->
                if (index == items.size - 1) {
                    ...
                    collectBatch2Music(playlists[index], musicList, success = {
                        ToastUtils.show("Song has been successfully added to the playlist ${playlists[index].name}")
                    })
                }
            }
        }
...
//            //新建歌单
        MaterialDialog(activity).show {
            title(text = "Whether to${musicList.size}Song import to new song list")
                ...
                createPlaylist(title, success = {
                    it.pid?.let { _ ->
                        collectBatch2Music(it, musicList, success = {
                            ToastUtils.show("Song has been successfully added to the playlist ${it.name}")
                        })
                    }
                }, type = playlist.type)
            }
    ...
    private fun showPlaylistSelectDialog(activity: AppCompatActivity, callBack: ((String) -> Unit)) {
        val items = mutableListOf("Local playlist", "Online playlist")
        MaterialDialog(activity).show {
            title(R.string.add_to_playlist)
            listItems(items = items) { dialog, index, text ->
    ...
    private fun collectMusic(playlist: Playlist, music: Music?) {
        if (playlist.type == Constants.PLAYLIST_LOCAL_ID) {
            playlist.pid?.let {
                ...
                ToastUtils.show("Added to local playlist successfully")
