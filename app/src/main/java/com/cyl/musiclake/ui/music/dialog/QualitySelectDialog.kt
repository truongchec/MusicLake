...
class QualitySelectDialog : BaseBottomSheetDialogFragment() {
    ...
    private fun initItems() {
        val qualities = mutableListOf(
                QualityItem(name = "Standard quality", quality = 128000))
        music?.let {
            if (it.high) {
                qualities.add(QualityItem(name = "Higher quality", quality = 192000))
            }
            if (it.hq) {
                qualities.add(QualityItem(name = "HD quality", quality = 320000))
            }
            if (it.sq) {
                qualities.add(QualityItem(name = "Lossless quality", quality = 999000))
            }
        }
